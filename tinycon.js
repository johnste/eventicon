...
(function(){
	...
	var removeFaviconTag = function(){
		...
		for(var i=0, len=links.length; i < len; i++) {
			...
			if (exists && (links[i].getAttribute('rel') === 'icon' || links[i].getAttribute('rel') === 'shortcut icon')) {
	...
	var getCurrentFavicon = function(){
		...
		for(var i in options.overrides) {
			if(options.overrides[i].origin == document.location.origin) {
	...
	var drawFavicon = function(label, colour) {
		...
		// Fixed code because of bug 238071 in Chrome 27
